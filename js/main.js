window.onresize = window.onload = function (){
	...
	// 	clearInterval(leader[0].time);
	// 	if(leader[0].offsetLeft == 0){
	// 		move(leader[0], -200, 10);
	// 	}else{
	// 		move(leader[0], 0, 10);
	// 	}
	// }
	var contentHeight = document.documentElement.clientHeight-header.offsetHeight + 'px'
	content.style.height = contentHeight;
	leader[0].style.height = contentHeight;
	view[0].style.height = contentHeight;
	viewer[0].style.height = document.documentElement.clientHeight-header.offsetHeight-70 + 'px';
	if(document.documentElement.clientWidth <= 940){
		viewer[0].style.width = viewer[0].offsetHeight * 4 / 3 + 'px';
		detail[0].style.display = 'none';
	}else if(document.documentElement.clientWidth> 940&&document.documentElement.clientWidth<= 1170){
		viewer[0].style.width = viewer[0].offsetHeight * 4 / 3 + 'px';
		detail[0].style.display = 'block';
	}else{
		viewer[0].style.width = viewer[0].offsetHeight * 16 / 9 + 'px';
		detail[0].style.display = 'block';
	}
	detail[0].style.height = viewer[0].offsetHeight + 'px';
	detail[0].style.width = document.documentElement.clientWidth-viewer[0].offsetWidth-105 + 'px';
	conList.style.height = viewer[0].offsetHeight - 145 + 'px';
	date.style.width = conList.offsetWidth + 'px';
	confirm.style.width = conList.offsetWidth + 'px';
	select();
